...
func setupDB() *sql.DB {
	...
	psqlconn := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable", HOST, PORT, DB_USER, DB_PASSWORD, DB_NAME)
...
func CreateMovie(w http.ResponseWriter, r *http.Request) {
	...
	// unMarshalling the json body of postman to get the attributes.
	body, _ := ioutil.ReadAll(r.Body) // check for errors

	keyVal := make(map[string]string)
	json.Unmarshal(body, &keyVal) // check for errors

	movieID := keyVal["movieid"]
	movieName := keyVal["moviename"]
...
func main() {
	...
	*/
	...
	// Create a movie
	router.HandleFunc("/create/movies/", CreateMovie).Methods("POST")
